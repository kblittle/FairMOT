...
class LinkData(Dataset):
    ...
    def initialize(self):
        id2info = dict()
        for seqid, seq in enumerate(os.listdir(join(self.root,self.mode)), start=1):
            path_gt = join(self.root, self.mode,seq,'gt/gt.txt')
...
if __name__ == '__main__':
    dataset = LinkData(
        root='/home/fc/datasets/sportsmot_data/images',
        mode='train'
    )
