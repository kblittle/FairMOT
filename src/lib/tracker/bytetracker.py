...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, temp_feat, buffer_size=30,temp_number_feat=None):
        ...
        self.curr_number_feat = None
        if temp_number_feat is not None:
            self.smooth_number_feat = None
            self.number_features = deque([], maxlen=buffer_size)
            self.update_number_features(temp_number_feat)
        else:
            self.smooth_number_feat = None
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        if new_track.curr_number_feat is not None:
...
class BYTETracker(object):
    def __init__(self, opt, frame_rate=30):
        ...
        if opt.use_gmc:
            self.gmc = GMC(method=opt.cmc_method)
    ...
    def update(self, im_blob, img0):
        ...
        if len(dets) > 0:
            ...
            if self.opt.fuse_number:
                for i in range(0, dets.shape[0]):
                    ...
                    number_feature_temp_flatten=number_feature_temp.flatten().detach().cpu()
        ...
        if self.opt.use_gmc:
            # Fix camera motion
            # gmc_image = torch.squeeze(img0, 0)
            # gmc_image=gmc_image.detach().cpu().permute(1, 2, 0).numpy()
            gmc_image=img0
            # plt.imshow(gmc_image)
            # plt.show()
            warp = self.gmc.apply(gmc_image, dets)
        ...
        if self.opt.use_EG_fuse:
            dists_giou = matching.iou_distance(strack_pool, detections, "giou")
            dists_giou = dists_giou.numpy() / 2
            dists_fuse = dists + dists_giou
            matches, u_track, u_detection = matching.linear_assignment(dists_fuse, thresh=self.opt.match_thres)
        else:
        ...
        number_feature_list_second=[]
        if len(dets_second) > 0:
            '''Detections'''
            # 如果dets_second不需要使用Re-ID，这里可以修改程序，从而加快速度
            if self.opt.fuse_number:
                for i in range(0, dets_second.shape[0]):
                    # get number image
                    bbox = dets_second[i][0:4]  # ltrb
                    height = bbox[3] - bbox[1]
                    width = bbox[2] - bbox[0]
                    left = width / 10 + bbox[0]
                    right = 9 * width / 10 + bbox[0]
                    ...
                    with torch.no_grad():
                        number_feature_temp = self.number_model.forward_features(number_image.to(self.opt.device))
                    number_feature_temp_flatten = number_feature_temp.flatten().detach().cpu()
                    number_feature_list_second.append(number_feature_temp_flatten)
                number_feature = torch.stack(number_feature_list_second, dim=0)
                number_feature = F.normalize(number_feature, dim=1)
                number_feature = number_feature.numpy()
                detections_second = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f, 30, number_f) for
                              (tlbrs, f, number_f) in
                              zip(dets_second[:, :5], id_feature, number_feature)]  # dets:[bboxes, scores, clses]
            else:
                detections_second = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f, 30) for
                              (tlbrs, f) in zip(dets_second[:, :5], id_feature)]  # dets:[bboxes, scores, clses]
        else:
            detections_second = []
        ...
        if self.opt.use_2d:
            ...
            detections = detections + detections_second
        else:
            detections = [detections[i] for i in u_detection]
